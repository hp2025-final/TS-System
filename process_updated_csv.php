...
// Process the UPDATED CSV file and create new normalized data files
...
echo "=== PROCESSING UPDATED DRESS DATA (SECOND UPDATE) ===\n\n";

// Process all rows from updated file
while (($data = fgetcsv($handle)) !== FALSE) {
    // Extract data from CSV
    $barcode = trim($data[0]);
    $productName = trim($data[1]); // Now includes size in name
    $sku = trim($data[2]);
    ...
    // Create unique dress key - using the full product name which now includes size
    $dressKey = $productName . '|' . $sku;
    if (!isset($dresses[$dressKey])) {
        $dresses[$dressKey] = [
            'collection' => $collectionName,
            'name' => $productName, // This now includes size like "Nooraya S"
...
echo "Updated Data Analysis:\n";
echo "=====================\n";
echo "Total Items: " . count($dressItems) . "\n";
...
foreach ($collections as $collection) {
    ...
    foreach ($dressItems as $item) {
        if ($dresses[$item['dress_key']]['collection'] === $collection['name']) {
            $itemCount++;
        }
    }
    echo "- {$collection['name']}: {$itemCount} items ({$collection['description']})\n";
}

echo "\nSample Items with Updated Names:\n";
$sampleCount = 0;
foreach ($dresses as $dress) {
    if ($sampleCount >= 5) break;
    echo "- {$dress['name']} - {$dress['collection']} - PKR {$dress['sale_price']}\n";
    $sampleCount++;
}

// 1. Create Collections CSV
echo "\nCreating collections_data_updated.csv...\n";
$collectionsFile = fopen('collections_data_updated.csv', 'w');
...
echo "Creating dresses_data_updated.csv...\n";
$dressesFile = fopen('dresses_data_updated.csv', 'w');
fputcsv($dressesFile, [
...
echo "Creating dress_items_data_updated.csv...\n";
$itemsFile = fopen('dress_items_data_updated.csv', 'w');
fputcsv($itemsFile, [
...
echo "\n=== UPDATED DATA FILES CREATED SUCCESSFULLY ===\n";
echo "Collections: " . count($collections) . "\n";
echo "Dresses: " . count($dresses) . "\n";
echo "Dress Items: " . count($dressItems) . "\n";
echo "\nFiles created:\n";
echo "- collections_data_updated.csv\n";
echo "- dresses_data_updated.csv\n";
echo "- dress_items_data_updated.csv\n";
